...
async function sendFcmNotification(tokens, payload, options = {}) {
  ...
  console.log('[FCM] Tokens:', tokens.map(t => t.substring(0, 30) + '...'));
  
  try {
    const messaging = admin.messaging();
    const app = admin.app();
    console.log('[FCM] Firebase app initialized, projectId:', app.options?.projectId || 'unknown');
    ...
    if (failureCount > 0) {
      const errors = results
        ...
        .map((r, idx) => `Token ${idx}: ${r.reason?.message || 'unknown error'}`)
        .join('; ');
